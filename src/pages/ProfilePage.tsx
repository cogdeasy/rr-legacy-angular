...
export default function ProfilePage() {
  ...
  const savePrefs = () => {
    setIsSaving(true);
    void updatePreferences(draftPreferences).then(() => {
      setIsSaving(false);
  ...
  const setDraftField = (field: keyof User, value: string) =>
    ...
    <>
      ...
      <div className="profile-layout">
        <aside className="panel identity-card">
          ...
          <span className="status-pill state-nominal">{user.authorityLevel}</span>
          ...
          <div className="panel">
            ...
            {!isEditing ? (
              ...
              <form className="detail-grid" onSubmit={saveProfile}>
                {(
                  ['firstName', 'lastName', 'email', 'phone', 'baseLocation'] as const
                ).map((field) => (
                  <label
                    className={`field${field === 'baseLocation' ? ' wide' : ''}`}
                    key={field}
                  >
                ...
                <div className="form-actions wide">
                  <button
                    className="rr-button rr-button-stroked"
                    type="button"
          ...
          <div className="panel">
            ...
            <label className="toggle-row">
              <input
                type="checkbox"
                name="alertEmails"
                checked={draftPreferences.alertEmails}
                onChange={(event) =>
                  setDraftPreferences({
                    ...draftPreferences,
                    alertEmails: event.target.checked
                  })
                }
              />
              <span>
            ...
            <label className="toggle-row">
              <input
                ...
                checked={draftPreferences.dailyFleetDigest}
                onChange={(event) =>
                  setDraftPreferences({
                    ...draftPreferences,
                    dailyFleetDigest: event.target.checked
                  })
                }
              />
              <span>
            ...
            <label className="toggle-row">
              <input
                type="checkbox"
                name="aogPager"
                checked={draftPreferences.aogPager}
                onChange={(event) =>
                  setDraftPreferences({
                    ...draftPreferences,
                    aogPager: event.target.checked
                  })
                }
              />
              <span>
                ...
                <span className="toggle-note">
                  Page immediately for aircraft on ground events.
                </span>
              </span>
            </label>
            <div className="threshold-row">
              <label className="field">
                ...
                <input
                  ...
                  value={draftPreferences.egtMarginThreshold}
                  onChange={(event) =>
                    setDraftPreferences({
                      ...draftPreferences,
                      egtMarginThreshold: Number(event.target.value)
                    })
                  }
                />
              </label>
              <label className="field">
                ...
                <select
                  name="defaultFleet"
                  value={draftPreferences.defaultFleet}
                  onChange={(event) =>
                    setDraftPreferences({
                      ...draftPreferences,
                      defaultFleet: event.target.value
                    })
                  }
                >
                  {user.fleetsCovered.map((fleet) => (
