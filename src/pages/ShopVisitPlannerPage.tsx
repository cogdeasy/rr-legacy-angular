...
const facilities: Facility[] = [
  { name: 'Derby', factor: 1, note: 'Home base, full module capability.' },
  {
    name: 'Dahlewitz',
    factor: 1.08,
    note: 'Specialist HP turbine cell, higher labour rate.'
  },
  {
    name: 'Singapore',
    factor: 0.94,
    note: 'Lower labour rate, longer parts lead time.'
  },
...
export default function ShopVisitPlannerPage() {
  const { data, isLoading } = useAsync(
    () =>
      ...
      }).then(async ({ engines, workscopeOptions }) => ({
  ...
  let consumed = 0;
  const donutSegments: DonutSegment[] = estimate.breakdown.map(
    (line: ShopVisitCostLine, index) => {
  ...
  const toggleWorkscope = (code: string) => {
    setSelectedCodes((codes) =>
      codes.indexOf(code) > -1
        ? codes.filter((selected) => selected !== code)
        : [...codes, code]
    );
  };
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <p>
            Build a workscope, pick a facility, and see the cost, downtime and
            margin recovered.
          </p>
        </div>
        <button
          className="rr-button rr-button-stroked pill-cta"
          onClick={() => setSelectedCodes([])}
        >
          <Icon>restart_alt</Icon> Reset workscope
        </button>
      </div>
          ...
          <section className="planner-inputs">
            <div className="panel">
              ...
              <select
                ...
                name="selectedEsn"
                value={selectedEsn || engines[0]?.esn || ''}
                ...
                <div className="engine-facts">
                  ...
                  <div>
                    ...
                    <strong className="numeric">
                      {selectedEngine.cyclesToShopVisit}
                    </strong>
                  </div>
                  <div>
            ...
            <div className="panel">
              ...
              {workscopeOptions.map((option: WorkscopeOption) => (
                <label className="workscope-option" key={option.code}>
                  <input
                    type="checkbox"
                    checked={selectedCodes.indexOf(option.code) > -1}
                  ...
                  <span className="option-body">
                    ...
                    <span className="option-meta numeric">
                      {option.labourHours} labour hrs ·{' '}
                      {currencyFormat(option.materialCost)} materials ·{' '}
                ...
                {facilities.map((facility) => (
                  <button
                    ...
                    className={`facility-chip${
                      facility.name === selectedFacility.name ? ' selected' : ''
                    }`}
          ...
          <section className="planner-output">
            <div className="panel estimate-panel">
              ...
              <p className="estimate-sub">
                {estimate.turnaroundDays} days turnaround · {estimate.labourHours}{' '}
                labour hours
              </p>
              <div className="estimate-grid">
                <div>
                  ...
                  <strong className="numeric">
                    {currencyFormat(estimate.labourCost)}
                  </strong>
                </div>
                <div>
                  <span className="micro-label">Materials</span>
                  <strong className="numeric">
                    {currencyFormat(estimate.materialCost)}
                  </strong>
                </div>
                <div>
                  <span className="micro-label">Margin recovered</span>
                  <strong className="numeric">
                    +{estimate.restoredEgtMargin} degC
                  </strong>
                </div>
                <div>
              ...
              {selectedEngine && (
                <p className="projection">
                  ...
                  <strong className="numeric">{projectedMargin} degC</strong>{' '}
                  margin.
                </p>
              )}
            </div>
            <div className="panel donut-panel">
              ...
              <div className="donut-row">
                <svg
                  ...
                  className="donut"
                  role="img"
                  aria-label="Cost split by workscope"
                >
                ...
                <ul className="legend">
                  {donutSegments.map((segment) => (
                    <li key={segment.label}>
                      ...
                      <span className="legend-share numeric">
                        {segment.share}%
                      </span>
                    </li>
                  ))}
