...
export default function WorkOrdersPage() {
  ...
  const loaded = useAsync(
    () =>
      Promise.all([fleetService.getWorkOrders(), fleetService.getEngines()]).then(
        ([workOrders, engines]) => ({ workOrders, engines })
      ),
    []
  );
  ...
  const openCount = workOrders.filter(
    (order) => order.status !== WorkOrderStatus.Closed
  ).length;
  const aogCount = workOrders.filter(
    (order) =>
      order.priority === WorkOrderPriority.Aog &&
  ...
  const updateField = <K extends keyof WorkOrderForm>(
    field: K,
    value: WorkOrderForm[K]
  ) => setForm((current) => ({ ...current, [field]: value }));
  const isInvalid = (field: keyof WorkOrderForm): boolean => {
    if (field === 'title') {
      return form.title.length < 8;
    }
    if (field === 'findings') {
      return form.findings.length < 12;
    }
    if (field === 'requesterEmail') {
      return !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.requesterEmail);
    }
    return !String(form[field]).trim();
  };
  const hasError = (field: keyof WorkOrderForm): boolean =>
    touched[field] === true && isInvalid(field);
  ...
  const taskProgress = (order: WorkOrder): number =>
    order.tasks.length === 0
      ? 0
      : Math.round(
          (order.tasks.filter((task) => task.complete).length /
            order.tasks.length) *
            100
        );
  const submit = (event: FormEvent) => {
    event.preventDefault();
    const fields: (keyof WorkOrderForm)[] = [
      'esn',
      'title',
      'dueOn',
      'findings',
    ...
    const nextTouched = fields.reduce(
      (all, field) => ({ ...all, [field]: true }),
      touched
    );
    setTouched(nextTouched);
    if (fields.some((field) => isInvalid(field))) {
      return;
    }
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <p>
            Track raised work, chase parts, and route new findings to a facility.
          </p>
        </div>
        <button
          className="rr-button rr-button-flat pill-cta"
          onClick={() => {
      ...
      {submittedId && (
        <div className="confirmation panel">
          <Icon>task_alt</Icon>
          <span>
            {submittedId} raised and routed to the facility planning queue.
          </span>
        </div>
      )}
      ...
      {!loaded.isLoading && <div>
        ...
        {showForm && (
          <form className="panel raise-form" onSubmit={submit}>
            ...
            <div className="form-grid">
              <label className="field">
                ...
                <select
                  value={form.esn}
                  onChange={(event) => updateField('esn', event.target.value)}
                  onBlur={() => setTouched({ ...touched, esn: true })}
                >
                  <option value="" disabled>
                    Select an engine
                  </option>
                  {engines.map((engine) => (
                    <option key={engine.esn} value={engine.esn}>
                ...
                {hasError('esn') && (
                  <span className="field-error">
                    Select the engine this work applies to.
                  </span>
                )}
              </label>
              <label className="field">
                ...
                {hasError('title') && (
                  <span className="field-error">
                    Give the work order a descriptive title.
                  </span>
                )}
              </label>
              <label className="field">
              ...
              <label className="field">
                ...
                <input
                  ...
                  onBlur={() => setTouched({ ...touched, dueOn: true })}
                />
                {hasError('dueOn') && (
                  <span className="field-error">A due date is required.</span>
                )}
              </label>
              <label className="field">
                ...
                <input
                  ...
                  onBlur={() => setTouched({ ...touched, requesterName: true })}
                />
              ...
              <label className="field">
                ...
                <input
                  ...
                  onBlur={() =>
                    setTouched({ ...touched, requesterEmail: true })
                  }
                />
            ...
            <label className="field wide">
              ...
              <textarea
                ...
                onBlur={() => setTouched({ ...touched, findings: true })}
              />
              {hasError('findings') && (
                <span className="field-error">
                  Record the supporting findings.
                </span>
              )}
            </label>
            <label className="field wide">
              <span className="field-label">
                Requested tasks (one per line)
              </span>
              <textarea
                rows={4}
                ...
                onChange={(event) =>
                  updateField('requestedTasks', event.target.value)
                }
                onBlur={() => setTouched({ ...touched, requestedTasks: true })}
              />
        ...
        <div className="filter-bar panel">
          ...
          <div className="chip-row">
            {statuses.map((status) => (
              <button
                key={status}
                className={`chip${status === selectedStatus ? ' selected' : ''}`}
            ...
            <article className="panel order-card" key={order.id}>
              ...
              >
                ...
                <div className="order-meta">
                  <span
                    className={`status-pill ${priorityClass(order.priority)}`}
                  >
                    {order.priority}
                  </span>
              ...
              <div className="order-progress">
                <div className="progress-track">
                  <div
                    ...
                    style={{ width: `${taskProgress(order)}%` }}
                  />
                </div>
                <span className="progress-value numeric">
                  {taskProgress(order)}% tasks complete
                </span>
              </div>
              {expandedId === order.id && (
                <div className="order-detail">
                  <div className="detail-facts">
                    ...
                    <div>
                      ...
                      <strong>
                        {formatDate(order.raisedOn, 'dd MMM yyyy')}
                      </strong>
                    </div>
                    <div>
                      <span className="micro-label">Labour booked</span>
                      <strong className="numeric">
                        {engineHours(order.labourHours)}
                      </strong>
                    </div>
                  </div>
