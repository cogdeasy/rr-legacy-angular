...
description: Browser-level testing of the RR Engine Operations Portal (React 18 + Vite) — how to boot the app, sign in past the mocked auth, reset in-memory state, and exercise each page's deterministic data.
---

# Testing the RR Engine Operations Portal

React 18 + Vite SPA with mocked auth and deterministic in-memory data. No backend, no network
calls — every number on screen is derived from `src/shared/data/fleetData.ts` and
`src/shared/data/fleetService.ts`.
...
Vite serves in ~1s; wait for the `ready in` line before driving the browser.

## Signing in

Auth is mocked in `src/shared/auth/authStore.ts`:
...
To force a logged-out state, clear the key or use the header user menu → Sign out. The
`RequireAuth` wrapper on every route bounces unauthenticated direct navigation back to `/login`.

## Resetting state between runs

All mutations (new work orders, profile edits, notification badge) live in module-level stores in
memory only. A **full page reload wipes them** — reload rather than trying to undo edits. Note
this also means a profile edit (e.g. renaming the user) will keep showing in the header and in the
dashboard greeting until you reload.

## Per-page notes

- **Every route shows a spinner first** (`Loading fleet health…`, `Loading engine register…`,
  etc.) because the data modules resolve through `delay(...)`. Always re-read the DOM after a
...
- **Shop visit planner**: cost/turnaround/restored-margin are pure functions of the selected
  workscope items × facility multiplier. Compute expected values from `fleetService.ts` rather
  than trusting the UI. The projection line should equal the engine's current margin + restored
  margin.
...
A clean run logs only the Vite dev-server connection message and **zero** errors or warnings.
